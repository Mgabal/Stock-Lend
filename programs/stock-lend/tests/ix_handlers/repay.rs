...
use crate::ix_handlers::init::{
    convert_account_metas, to_anchor_pubkey, TestMarket, BORROW_VAULT_SEED, COLLATERAL_VAULT_SEED,
    USER_POSITION_SEED,
};
...
pub fn repay(
    ...
    let accounts = stock_lend::accounts::Repay {
    ...
    let ix_data = stock_lend::instruction::Repay { amount };
...
pub fn withdraw_collateral(
    ...
    let accounts = stock_lend::accounts::WithdrawCollateral {
        user: to_anchor_pubkey(user.pubkey()),
        user_position: to_anchor_pubkey(user_position),
        market: to_anchor_pubkey(market.market),
    ...
    let ix_data = stock_lend::instruction::WithdrawCollateral { amount };
